...
notify_paths = {"path": ["/orders"], "method": ["POST"]}


def notify(request):
    if request.path in notify_paths["path"] and request.method in notify_paths["method"]:
        # message = {"path": request.path, "method": request.method, "msg": "Hi, a new order has been created!"}
        message = "Hi, a new order has been created!"
        # print(message)
